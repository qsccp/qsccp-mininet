...
def run():
    ...
    addRoute(ndn, "c4", "r1", "/D")
    
    addRoute(ndn, "r1", "r2", "/A")
    addRoute(ndn, "r1", "r2", "/B")
    addRoute(ndn, "r1", "r2", "/C")
    addRoute(ndn, "r1", "r2", "/D")
    
    addRoute(ndn, "r2", "r3", "/A")
    addRoute(ndn, "r2", "r3", "/B")
    addRoute(ndn, "r2", "r3", "/C")
    addRoute(ndn, "r2", "r3", "/D")
    
    addRoute(ndn, "r3", "r4", "/A")
    addRoute(ndn, "r3", "r4", "/B")
    addRoute(ndn, "r3", "r4", "/C")
    addRoute(ndn, "r3", "r4", "/D")
    
    addRoute(ndn, "r4", "p1", "/A")
    addRoute(ndn, "r4", "p2", "/B")
    addRoute(ndn, "r4", "p3", "/C")
    addRoute(ndn, "r4", "p4", "/D")
    ...
    ping1 = getPopen(
        ndn.net["c1"],
        f"qsccp-client --prefix /A --timingStop {total_time} --fixedRate 1000000 --tos 2",
    ...
    ping1 = getPopen(
        ndn.net["c2"],
        f"qsccp-client --prefix /B --timingStop {total_time} --tos 5 --initialSendRate 50000",
        stdout=qsccp_client_log,
        stderr=qsccp_client_log,
    )
    ...
    ping1 = getPopen(
        ndn.net["c3"],
        f"qsccp-client --prefix /C --timingStop {total_time} --tos 6 --initialSendRate 50000",
        stdout=qsccp_client_log,
        stderr=qsccp_client_log,
    )
    qsccp_client_log = open(f"{ndn.workDir}/c4/qsccp-client.log", "w")
    ping1 = getPopen(
        ndn.net["c4"],
        f"qsccp-client --prefix /D/1 --timingStop {total_time} --tos 7 --initialSendRate 50000",
        stdout=qsccp_client_log,
        stderr=qsccp_client_log,
    )
    ping1.wait()
